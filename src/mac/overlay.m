    ...
    app_dispatch_main(^{
        @autoreleasepool {
            ...
            if (!appIcon) {
                ...
                appIcon = [NSImage imageWithSystemSymbolName:@"mic.fill" 
                                   accessibilityDescription:@"Recording"];
                if (appIcon) {
                    // Configure the SF Symbol
                    NSImageSymbolConfiguration* config = [NSImageSymbolConfiguration 
                        configurationWithPointSize:16 weight:NSFontWeightRegular];
            ...
            if (appIcon) {
                ...
                if ([appIcon isTemplate]) {
                    [iconView setContentTintColor:[NSColor colorWithRed:0x22/255.0 green:0xC5/255.0 blue:0x5E/255.0 alpha:1.0]];
                }
    ...
    app_dispatch_main(^{
        @autoreleasepool {
            ...
            if (overlayWindow && messageLabel) {
                ...
                NSAttributedString *attributedText = [[NSAttributedString alloc] initWithString:messageString attributes:attributes];
                ...
                NSRect screenFrame = mainScreen.frame;
                NSPoint position = NSMakePoint(
                    (screenFrame.size.width - windowWidth) / 2,
                    30  // 30 pixels from bottom
                );
                [overlayWindow setFrameOrigin:position];
